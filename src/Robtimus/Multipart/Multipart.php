...
abstract class Multipart
{
    ...
    {
        $headerValue = $this->escapeHeaderValue($type, false);
        if ($name !== '') {
            $headerValue .= '; name="' . $this->escapeHeaderValue($name, false) . '"';
        }
        if ($filename !== '') {
            $headerValue .= '; filename="' . $this->escapeHeaderValue($filename, false) . '"';
        }
        $this->addHeader('Content-Disposition', $headerValue);
    }
    ...
    {
        $this->addHeader('Content-ID', $this->escapeHeaderValue($contentID, false));
    }
    ...
    {
        $this->addHeader('Content-Type', $this->escapeHeaderValue($contentType, true));
    }
    ...
    {
        $this->addHeader('Content-Transfer-Encoding', $this->escapeHeaderValue($contentTransferEncoding, false));
    }
     ...
     * @throws LogicException If the multipart is already finished.
     */
    private function addHeader(string $headerName, string $headerValue): void
    {
     ...
     * @return string
     */
    private function escapeHeaderValue(string $value, bool $allowQuotes): string
    {
     ...
     * @throws LogicException If the multipart is already finished.
     */
    private function add(mixed $part, int $length = -1): void
    {
        if ($this->finished) {
     ...
     * @throws UnexpectedValueException If any resource part is no longer readable.
     */
    private function doRead(int $length): string
    {
     ...
     * @throws UnexpectedValueException If any resource part is no longer readable.
     */
    private function doReadFromPart(int $length): string
    {
     ...
     * @throws UnexpectedValueException If any resource part is no longer readable.
     */
    private function doBuffer(int $bufferSize = 8192): string
    {
        if (!$this->isBuffered()) {
            $this->index = 0;
            $this->partIndex = 0;
        ...
        // when buffered, $this->parts is an array with a single string element
